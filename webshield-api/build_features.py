...
def _crs_features(request_text):
    x = pd.DataFrame(index=request_text.index)
...
def build_features(df, feature_names=None):
    ...
    parts = [
        _baseline_features(df, request_text),
        _crs_features(request_text),
        _ua_features(df),
        pd.get_dummies(df[["Method", "Host-Header"]], drop_first=True, dtype=int),
    ]
